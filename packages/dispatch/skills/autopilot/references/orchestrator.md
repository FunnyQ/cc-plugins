...
// Only a live pane can ask: `codex exec` is handed no such tool, so a headless
// delegate genuinely cannot. The TUI relay spawns does — measured 2026-09-18, six
// `request_user_input_async` calls across five panes of one flight. The point of a
// flight is that it runs semi-unattended, and each of those six stopped to consult
// the one person who should not have had to watch. Five were answered and those
// tasks landed correctly, so this is not a correctness bug — it is the loss of the
// property autopilot exists to provide. The sixth went unanswered: that engine said
// it would keep waiting, and its rollout ends with no `task_complete` at all, which
// is why the rule below bans waiting for a reply and not merely asking for one.
const NO_ASK_RULE = `Nobody is watching this run, so do not call any ask, clarify, or request-user-input tool, and never wait on a reply to one. Where the task is ambiguous, implement exactly what the task file specifies. Where something genuinely blocks you, never substitute your own scope, framework, or gate to get past it — finish what you can, then name the blocker in your final message, which is the only channel anyone reads.`
