...
// Gates only the dev driver's delegate command below; the review lens always stays headless.
...
4. Write that instruction to a temp file, then run:
  ${liveDev ? `bun ${CFG.relayPath} ${engine.label} delegate --prompt-file <your-instruction-file> --git-scope none --dangerous --wait-timeout 900000` : `bun ${S}/${engine.wrapper} delegate${engine.modelFlag} --prompt-file <your-instruction-file>`}
...
5. If the command exits non-zero${liveDev ? ' or relay returns an empty/absent result or a PENDING report on stdout ("still running after ... this is NOT a failure")' : ` or its output begins with "${engine.token}"`}, do NOT hand-write the implementation yourself. ${liveDev ? 'For a relay PENDING report, log the failure (step 7) as a live-pane timeout, include the Agent name from relay stdout, and note the pane was LEFT OPEN for postmortem; return failure so this gate cycle retries or escalates. ' : ''}Log the failure (step 7) and return a summary stating ${engine.label} was unreachable or failed — the binary gate will then fail this attempt and the loop moves on (the final attempt escalates to Claude-Opus automatically). Step 6 remains the real correctness gate either way; incomplete or timed-out work fails verification.
...
- **`CFG.devEngine: 'codex'` (or `'opencode'`) hands the dev step to that CLI.** Default is `'claude'` (Sonnet→Opus). When set to an external engine, each non-finalReview task's dev step becomes a cheap Haiku *driver* that runs the `<engine>-run.ts delegate` wrapper (reachable from a Workflow agent's Bash — the same wrapper the cross-vendor review lens uses), so the external CLI writes the implementation. If `CFG.liveDevEngine` is true and `CFG.relayPath` resolved under `HERDR_ENV=1`, the driver instead runs the same delegate through relay in a visible herdr live pane; otherwise it uses the headless wrapper exactly as before. The verify → judge → score pipeline stays Claude, which *strengthens* the dev≠judge split into a cross-vendor one (the external CLI writes, Claude-Opus judges). The last attempt before the cap still falls back to Claude-Opus, so a task the external engine can't clear gets one strong Claude try before parking; if the CLI is unreachable or relay returns no result, the driver never fabricates, the binary gate fails the attempt, and the loop reaches that Opus fallback. The live path passes a generous `--wait-timeout 900000` (15 min): relay's poll loop returns the instant the result lands, so a bigger budget costs nothing for normal-speed tasks and only extends patience for genuinely-long ones. A `pending` outcome therefore means the pane stayed actively *working* past 15 min — a pathological/runaway task, not a slow-but-fine one (a stuck-waiting agent settles to `idle`/`done` and is caught as a failure sooner), so treating pending as a failed attempt that retries/escalates is correct rather than waiting indefinitely. `devEngine` and `reviewEngine` are independent — you can have opencode write and codex review, or vice versa. Only the dev step changes — finalReview's multi-lens round and everything else are untouched.
