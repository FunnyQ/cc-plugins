...
const CFG = {
  ...
  liveCollectRounds:     3,       // live dev only: how many extra `relay collect` windows (8 min each) to keep waiting for a slow delegate before failing the attempt; 0 = fail as soon as relay reports pending
...
const withModel = (key, override) => {
  if (!ENGINES[key]) throw new Error(`unknown engine "${key}" — CFG.devEngine/reviewEngine must be codex or opencode`)
