...
- **cockpit** — the windshield: one skill with a thin `SKILL.md` router. Plain `/cockpit` routes through the provider reference (`references/claude-cli.md` or `references/codex.md`) into `references/pilot.md`; `/cockpit scribe` routes into `references/scribe.md` for auto-distilling work into typed decision-trail entries; `/cockpit restart` routes into `references/restart.md` (bounce the daemon onto this install's code — the `cockpit.ts restart` command below). The cockpit provides a distilled decision trail, live transcript, a `needs_your_call` wait/send bridge, and a send box for running sessions. Its dashboard daemon owns the live transcript view that usage-dashboard's "Live now" rows link into. Claude Code sends use the cockpit channel MCP server; Codex sends use the managed Codex remote-control app-server socket, with direct app-server as fallback. The channel is UI→agent only: the agent's answers ride the transcript (the single source of truth — no separate reply tool). **`needs_your_call` is presence-gated**: the TUI is the default asking surface, and cockpit is used only when the user has said they want it. `cockpit wait` passes `require_watcher=1`, and `/api/wait` refuses with `{not_watching:true, reason}` (CLI exit `4`) unless **two** factors hold — **intent**, the user's explicit `answer_here` switch (global, default off, in the XDG config; dashboard toggle + `cockpit config --answer-here on|off` + `GET/POST /api/answer-here`), and **liveness**, `hasVisibleSubscriber()` in `permission.ts` seeing a live permission-stream subscriber for that session. Intent can't be inferred: **`document.hidden` stays false when another app covers the browser** (verified — a minimized/backgrounded window still reported the tab visible), so a visibility-only signal never goes false for a user working in the terminal. Liveness still matters on its own: with the switch on and no tab connected, a park would hang forever. The gate sits *after* the stash drain and the superseded check, so a fast answer still lands and a moot call still reports `superseded`. The permission relay is deliberately **not** gated: its protocol is notification-based, the terminal prompt is always live alongside the cockpit card, so it already defaults to the TUI.
- **install** — one-stop setup (command-triggered): the canonical home for all prerequisite checks and config wiring for the whole plugin. `setup.ts` checks both skills and wires the one config a non-dev user can't easily edit by hand — the statusline collector in `~/.claude/settings.json`. (The **cockpit channel** is now packaged in the plugin manifest — `mcpServers` + `channels` in `.claude-plugin/plugin.json` — so it no longer needs a hand-written `~/.claude.json` entry; setup.ts only *cleans up* a stale entry left by older versions, which would otherwise double-register the channel.) The dashboard precheck (`install.ts`) and statusline wiring (`setup-statusline.ts` + pure `statusline-decision.ts`) live here; usage-dashboard imports them rather than owning copies. The Claude manifest has two **`SessionStart` hooks**: one runs `setup.ts --session-check` — marker-gated via `$CLAUDE_PLUGIN_DATA/.wired-version`, so once per version it silently re-points a version-drifted statusline path (the cache encodes the version, e.g. `.../monitor/3.1.0/...`, and old dirs linger so "wired" means *exact current path*, not mere existence) and removes any stale channel entry, or, on a fresh install, prints one write-free nudge to run `/monitor:install`; the other injects thoughtful auto-logging guidance for Claude sessions. It never fresh-wires the statusline — initial opt-in stays manual. Codex registers the **same two SessionStart hooks** (plus the `scribe-nudge.ts` Stop hook) via `.codex-plugin/hooks.json`, so codex sessions get the decision-log guidance too; `/thoughtful` on Codex only re-affirms it (there's no auto opt-in to flip). **Codex hook-env gotcha:** codex runs plugin hooks under its long-lived `codex app-server daemon`, whose environment is frozen at daemon-start. So a delegation env var like relay's `RELAY_DELEGATED=1` (which `decision-log-start.ts` / `scribe-nudge.ts` check to suppress nudges for delegated/subagent sessions) reaches the codex *frontend* but **not** the hook if the daemon predates it — the start hook fires anyway. Restarting the app-server refreshes the daemon env and fixes it. This is why the same `RELAY_DELEGATED` suppression that works cleanly on Claude Code can appear broken on Codex until the daemon is bounced.

The decision trail is the one thing cockpit writes into a repo, and it is anchored per **repo**, not per cwd: `log-root.ts` walks up from cwd for an existing `.cockpit/`, bounded by the git root, then falls back to the git root (and to cwd outside a repo). So an agent that cd'd into `frontend/` still logs to the root trail, while a hand-made `packages/x/.cockpit` keeps its own. The walk-up must never cross the git root — `~/.cockpit` is a real leftover of the pre-XDG cockpit home, and an unbounded walk would collapse every repo under `$HOME` into one trail. Session *lookup* keeps the raw cwd (find-session resolves sessions BY cwd), and for a tracked session the registry entry's absolute `logPath` — not the request's `project` param — is authoritative in `log-stream.ts` / `project-info.ts` / `design-system.ts`.
...
│   ├── .codex-plugin/plugin.json     # Codex manifest (skills: "./skills/" — both auto-discovered) + hooks → ./.codex-plugin/hooks.json (same SessionStart pair + Stop scribe-nudge as Claude)
│   ├── .codex-plugin/hooks.json      # Codex SessionStart (setup.ts --session-check + decision-log-start.ts) + Stop (scribe-nudge.ts) — mirrors the Claude manifest hooks
│   ├── commands/
...
# The "Ask me here" switch — where the user wants needs_your_call questions to
# land. Off (default) = the terminal. On = the cockpit, provided a tab is also
# connected. Same global XDG config as the language.
bun packages/monitor/skills/cockpit/scripts/cockpit.ts config get-answer-here
...
**Every plugin is versioned independently, on its own cadence.** There is no repo-wide version. Each plugin owns its version in its two `plugin.json` files and releases under a **plugin-scoped tag** `<plugin>-vX.Y.Z` (e.g. `chronicle-v0.1.0`). Current versions: monitor `3.22.0`, dispatch `3.16.0`, relay `0.6.0`, chronicle `0.9.6`, herdr `0.2.2`.

**This repo runs GitHub Flow: `main` is the only long-lived branch.** There is no `develop`. Work happens on `main` or on a short-lived branch merged back into it. Both `.chronicle/release.json` and `.chronicle/pr.json` record `"workflow": "github-flow"`, so `/chronicle:release` commits the bump on `main`, cuts every tag on **that bump commit**, and merges nothing.
...
**Preferred path: `/chronicle:release`** — this repo dogfoods its own release skill. Its committed `.chronicle/release.json` records the per-component shape: each plugin is an independently-versioned component, with its two `plugin.json` files as version-file patterns. So the skill bumps the right files, prepends the per-plugin `CHANGELOG.md` entry, and — in `auto` / `auto push` mode — commits the bump on `main`, tags it, and pushes. Pick the touched component(s) at its version gate. It supports **coordinated multi-component releases natively**: name several components, or select the changed set at the gate, and the finisher cuts N scoped tags on the one bump commit. The `dispatch 3.16.0 + monitor 3.22.0 + relay 0.6.0` release took exactly that shape: one bump commit on `main`, three scoped tags on it.

If cutting a release by hand instead: **bump the touched plugin's two `plugin.json` files by hand** to match its scoped tag, then add the matching `CHANGELOG.md` entry. Head it per-plugin, e.g. `## [chronicle 0.1.0]`, and note the scoped tag it tracks. Then commit on `main`, cut an annotated `<plugin>-vX.Y.Z` tag on that commit, and push `main` plus the tag.
