...
- **`serveStaticFile`'s ETag is built from mtime + size, never from the bytes.** Hashing content would re-read every file on every revalidation — the exact cost the 304 exists to avoid, and cockpit's mermaid bundle alone is 3.16MB. It is weak (`W/`) because mtime + size is not proof of identical content, and the encoding is part of the key, so a client holding the gzip validator does not get a 304 for the plain body. `Cache-Control` stays `no-cache`: that means *revalidate every time*, not *never cache*, and the ETag is what turns each of those revalidations into a 304. A warm reload drops from 0.98MB to 0.57MB, 33 of 42 requests answered 304.
- **`/api/stats` revalidates on the fingerprint that already keys its cache**, so a reload with an unchanged corpus costs the ~125ms fingerprint scan and a 304 instead of 0.58MB — 12.8KB for the whole page. `statsEtag()` in `api.ts` prefixes it with a **per-process `BOOT_ID`**, and that prefix is not optional: pricing partly resolves from a live OpenRouter fetch, which is not a file and never moves the fingerprint, so without it a browser would 304 straight past a restart that repriced. The tag means "the payload *this* process would build" — exactly what the in-memory cache means. `Cache-Control` had to become `no-cache`; `no-store` forbids the client from keeping a body to revalidate at all.
