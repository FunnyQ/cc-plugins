...
### What an earlier attempt already landed

Attempt 3 delivered every cockpit / install / reference edit and stopped one sentence short: it left the S5 background-launch fact out of `usage-dashboard/SKILL.md`, because S5b was unresolved at the time. S5b is now resolved (no background parameter exists; detach with `… &`), so that sentence is the outstanding work. The rest of this task's changes are already committed — confirm them rather than rewriting them, and keep the diff additive.
