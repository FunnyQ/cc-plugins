...
- **Session-hook output reaches the user, not the agent's context.** Both session handlers write to `console.error`. The plugin context exposes `{client, project, worktree, directory, experimental_workspace, serverUrl, $}` (S11) with no verified way to inject additional context into a session, and guessing one is exactly what `runtime-facts.md` forbids. So the OpenCode decision-log start is advisory text, not injected guidance, and is weaker than its Claude Code counterpart. Resolving this needs a live session probe of `client`.
