...
- **S5c** — the file tools name their arguments in **camelCase**: `write` takes `filePath` and `content`; `edit` takes `filePath`, `oldString`, and `newString`. Claude Code's `file_path` / `old_string` / `new_string` do **not** work, so a hook that reuses a Claude payload shape must translate (`commentPayload` in `plugin.ts` is that translation). Read the names off the binary rather than guessing — a wrong name is silently empty, not an error, which ports a hook that never fires: `grep -ao oldString "$(readlink -f "$(which opencode)")" | wc -l` returns 41 against 1 for `old_string`.
